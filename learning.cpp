	...
	for (int data_num = 1; data_num <= data_siz; data_num++)
	{
		...
		for (int layer_num = 1; layer_num <= 3; layer_num++)
		{
			...
			else if (layer_num == 3)
			{
				...
				//対数尤度*(-1)を計算する(-1をかけることでこれを最小化できれば対数尤度が最大化するため)
				for (int class_num = 1; class_num <= class_siz; class_num++)
				{
...
void backward(
	...
	double denom = 1e-7;
	vector<double> gamma(data_siz + 10, 0);
	vector<double> demon(data_siz + 10 ,0);
	...
	for (int data_num = 1; data_num <= data_siz; data_num++)
	{
		for (int now_node = 1; now_node <= non_linear_input_siz; now_node++)
		{
			for (int class_num = 1; class_num <= class_siz; class_num++)
			{
				for (int component_num = 1; component_num <= each_class_siz[class_num]; component_num++)
				{
					demon[data_num] += pow((output_layer[data_num][class_num] - T_data[data_num].output[class_num])
						* mid_layer_output[data_num][class_num][component_num] / output_layer[data_num][class_num] * input_layer[data_num][now_node], 2);
				}
			}
		}
	}
	double tmp = pow(log_likelihood, beta);
	for (int data_num = 1; data_num <= data_siz; data_num++)
	{
		gamma[data_num] = tmp / demon[data_num];
	}
			...
			for (int component_num = 1; component_num <= each_class_siz[class_num]; component_num++)
			{
				...
				for (int data_num = 1; data_num <= data_siz; data_num++)
				{
					cnt += (output_layer[data_num][class_num] - T_data[data_num].output[class_num])
						...
						* gamma[data_num] * study_rate;
				}
				//cnt /= data_siz;
				weight[now_node][class_num][component_num] -= (before[now_node][class_num][component_num] + cnt) * sampling_time / 2;
				before[now_node][class_num][component_num] = cnt;
			}
		}
	}
}
